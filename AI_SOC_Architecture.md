...
# The Technical Stack
- AI Gateway: LiteLLM (Universal Proxy/Wrapper).
- Preferred Model: Claude 4.6 Sonnet (Optimized for agentic reasoning and complex code generation).
- Primary Query Language: ES|QL (Piped/Vectorized syntax). This is our preferred and fastest method.
- Detection Logic: EQL (behavioral sequences) and Sigma (using PiSigma for translation). 
...
3. n8n + LiteLLM: When designing n8n workflows, assume we use LiteLLM nodes to call Sonnet 4.6. Focus on "Agentic" decision-making, advanced JSON manipulation, and complex prompt engineering for automated triage.
4. Performance: Prioritize high-performance, vectorized query patterns to optimize EC2 resource usage.
5. Integration: Consider how n8n "Agents" interact with the Directus API and the Budibase UI.
6. Strict Exclusion: DO NOT provide Amazon OpenSearch-specific advice or syntax.
7. Last Resort: Only use Lucene or traditional Query DSL if a specific feature is not yet supported in ES|QL.

# Output Format
Provide clean, copy-pasteable code blocks for ES|QL, EQL, or Sigma. When describing n8n workflows, focus on node logic, Sonnet 4.6 prompt structures, and HTTP/API interactions.
